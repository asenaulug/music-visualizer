...
void draw_plasma_frame() {
    static int           x1, x2, x3, x4, y1, y2, y3, y4, sx1, sx2, sx3, sx4;
    ...
    for(y=0; y<_matrix_height; y++) {
      ...
      for(x=0; x<_matrix_width; x++) {
        value = hueShift
          ...
          + (UINT8)pgm_read_byte(sinetab + (UINT8)((x2 * x2 + y2 * y2) >> 2))
          + (UINT8)pgm_read_byte(sinetab + (UINT8)((x3 * x3 + y3 * y3) >> 3))
          + (UINT8)pgm_read_byte(sinetab + (UINT8)((x4 * x4 + y4 * y4) >> 3));
